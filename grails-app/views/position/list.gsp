			...
			<table>
				...
				<g:each in="${positionInstanceList}" status="i" var="positionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${positionInstance.id}">${fieldValue(bean: positionInstance, field: "beleg")}</g:link></td>
					
						<td>${fieldValue(bean: positionInstance, field: "anmerkung")}</td>
					
						<td>${fieldValue(bean: positionInstance, field: "menge")}</td>
					
						<td><g:formatDate date="${positionInstance.datum}" /></td>
					
						<td>${fieldValue(bean: positionInstance, field: "kunde")}</td>
					
						<td>${fieldValue(bean: positionInstance, field: "preis")}</td>
